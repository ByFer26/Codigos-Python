...
def iniciarSPI():
    enviar=f"acuse${str(chr(int(esclavo.get())))}"
...
esclavo=customtkinter.CTkSlider(ventana,orientation="horizontal",from_=33,to=125,number_of_steps=92,command=insertarDireccion)
...
direccionEsclavo.place(x=550,y=50)

texto1=customtkinter.CTkLabel(ventana,width=60,height=40,font=('helvetica',20),fg_color='transparent',text="Acuse recibido:")
...
botonEstablecerDireccion=customtkinter.CTkButton(ventana,width=100,height=50,text="Configurar SPI",font=('helvetica',18),command=iniciarSPI)
