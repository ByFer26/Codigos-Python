...
def crearTabla(nombre, col1, col2, col3):
    cursor = conexion.cursor()
    query = f'CREATE TABLE {nombre}({col1} varchar(30),{col2} varchar(30),{col3} varchar(30))'
    cursor.execute(query)
    cursor.close()

def insertarDatos(tabla, col1, col2, col3, nombre, apellido, estado):
    cursor = conexion.cursor()
    query = f"INSERT INTO {tabla}({col1},{col2},{col3}) VALUES ('{nombre}','{apellido}','{estado}')"
    cursor.execute(query)
    cursor.close()
